import { Vector3, Mesh, Matrix4, Ray, Triangle, Vector4, Line3, DoubleSide, Plane, Matrix3, BufferAttribute, BufferGeometry, Group, Color, MeshPhongMaterial, MathUtils, LineSegments, LineBasicMaterial, InstancedMesh, SphereBufferGeometry, MeshBasicMaterial } from "https://cdn.skypack.dev/three@0.145.0";
...
class CullableTriangle extends Triangle {
    ...
    updateSide(plane, triangle = null, coplanarIndex = -1) {
        ...
        _vec.copy(this.a).add(this.b).add(this.c).multiplyScalar(1 / 3);
    ...
    splitByPlane(plane, triangle = null, coplanarIndex = -1) {
        ...
        for (let i = 0, l = triangles.length; i < l; i++) {
            ...
            if (splittingTriangle) {
                if (!splittingTriangle.intersectsTriangle(tri, _edge$1) || _edge$1.distance() < 1e-5) {
...
function performWholeTriangleOperations(a, b, splitTriSet, operation, invert, attributeInfo, groupOffset = 0) {
    ...
    while (traverseSet.size > 0) {
        ...
        while (stack.length > 0) {
            ...
            switch (action) {
                case ADD_TRI:
                    appendAttributesFromIndices(i02, i12, i22, aAttributes, a.matrixWorld, _normalMatrix, attrSet);
                    break;
                case INVERT_TRI:
                    appendAttributesFromIndices(i22, i12, i02, aAttributes, a.matrixWorld, _normalMatrix, attrSet, invert);
                    break;
            }
        }
    }
...
function generateRandomTriangleColors(geometry) {
    ...
    for (let i = 0, l = array.length; i < l; i += 9) {
        color.setHSL(Math.random(), MathUtils.lerp(0.5, 1, Math.random()), MathUtils.lerp(0.5, 0.75, Math.random()));
        array[i + 0] = color.r;
        array[i + 1] = color.g;
        array[i + 2] = color.b;
        array[i + 3] = color.r;
...
class PointsHelper extends InstancedMesh {
    ...
    setPoints(points) {
        for (let i = 0, l = points.length; i < l; i++) {
            ...
            _matrix$2.makeTranslation(point.x, point.y, point.z);
            this.setMatrixAt(i, _matrix$2);
        }
    ...
    setHalfEdges(geometry, halfEdges) {
        ...
        for (let triIndex = 0; triIndex < triCount; triIndex++) {
            ...
            for (let e = 0; e < 3; e++) {
                ...
                _plane$1.projectPoint(_center2, _projected);
                ...
                _vec$1.addVectors(_center, _projectedDir);
                _vec2.addVectors(_center2, _projectedDir2);
                _tri1.closestPointToPoint(_vec$1, _finalPoint);
